...
import {
  type FieldAttribute,
  getAITables,
  type UnblockedDbSchema,
} from 'unblocked/db';
import type { UnblockedOptions } from 'unblocked/types';
...
export const generateDrizzleSchema: SchemaGenerator = async ({
  ...
  const filePath = file || './ai-schema.ts';
  const databaseType: 'sqlite' | 'mysql' | 'pg' | undefined =
    adapter.options?.provider;

  if (!databaseType) {
    throw new Error(
      'Database provider type is undefined during Drizzle schema generation. Please define a `provider` in the Drizzle adapter config. Read more at https://unblocked.dev/docs/adapters/drizzle'
    );
  }
