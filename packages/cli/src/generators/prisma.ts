...
import { capitalizeFirstLetter } from 'unblocked';
import { type FieldType, getAITables } from 'unblocked/db';
      ...
      if (!prismaModel) {
        if (provider === 'mongodb') {
          ...
          builder
            ...
            .attribute('id')
            .attribute(`map("_id")`);
        } else if (options.advanced?.database?.useNumberId) {
          builder
            .model(modelName)
            .field('id', 'Int')
            .attribute('id')
