...
export async function generateAIConfig({
  ...
  const _start_of_plugins_common_index = {
    START_OF_PLUGINS: {
      type: 'regex',
      regex: /unblocked\([\w\W]*plugins:[\W]*\[()/m,
  ...
  const common_indexes = {
    ...
    START_OF_UNBLOCKED: {
      ...
      regex: /unblocked\({()/m,
      getIndex: ({ matchIndex }) => {
        return matchIndex + 'unblocked({'.length;
      },
    } satisfies CommonIndexConfig<{}>,
  };
    ...
    add_database: async (opts: {
      ...
      async function add_db({
        ...
        if (code_before_betterAuth) {
          const start_of_unblocked = getGroupInfo(
            opts.config,
            common_indexes.START_OF_UNBLOCKED,
            {}
          );
          if (!start_of_unblocked) {
            throw new Error("Couldn't find start of unblocked() function.");
          }
          opts.config = insertContent({
            line: start_of_unblocked.line - 1,
            character: 0,
            content: opts.config,
            insert_content: `\n${code_before_betterAuth}\n`,
          });
      ...
      if (opts.database === 'sqlite') {
        ...
        await add_db({
          ...
          imports: [
            {
              path: 'unblocked/adapters/drizzle',
              variables: [
                {
                  name: 'drizzleAdapter',
                },
        ...
        await add_db({
          ...
          imports: [
            {
              path: 'unblocked/adapters/prisma',
              variables: [
                {
                  name: 'prismaAdapter',
                },
      ...
      let new_content: string;
      new_content = insertContent({
        line: start_of_unblocked.line,
        character: start_of_unblocked.character,
        content: opts.config,
