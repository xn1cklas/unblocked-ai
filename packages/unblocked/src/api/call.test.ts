...
// import { bearer } from "../plugins/bearer"; // Not available in unblocked
...
describe('call', async () => {
  ...
  const options = {
    ...
    plugins: [testPlugin, testPlugin2], // bearer() not available in unblocked
    hooks: {
  ...
  it('global before hook should change the context', async (ctx) => {
    // This test originally tested auth-specific behavior
    // Now testing that our test middleware can modify context
    const response = await api.test({
      query: {
        testContext: 'modified-value',
      },
    });
    expect(response.success).toBe('modified-value');
  });
