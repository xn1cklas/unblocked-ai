...
import { type AdapterDebugLogs, createAdapter } from '../create-adapter';
  ...
  createAdapter({
    ...
    adapter: ({ getFieldName, debugLog }) => {
      ...
      const withReturning = async (
        ...
        if (builderVal && builderVal[0]?.id?.value) {
      ...
      function checkMissingFields(
        ...
        for (const key in values) {
          if (!schema[key]) {
            throw new UnblockedError(
              `The field "${key}" does not exist in the "${model}" schema. Please update your drizzle schema or re-generate using "npx @unblocked/cli generate".`
            );
          }
        }
      }
