...
import type { Adapter, UnblockedOptions, Where } from '../../types';
...
export const createAdapter =
  ...
  (options: UnblockedOptions): Adapter => {
    const config = {
      ...cfg,
    ...
    // End-user's Unblocked instance's schema
    const schema = getAITables(options);
    ...
    const getDefaultFieldName = ({
      ...
      // Plugin `schema`s can't define their own `id`. Unblocked auto provides `id` to every schema model.
    ...
    const idField = ({
      ...
      return {
        ...
        ...(shouldGenerateId
          ? {
              defaultValue() {
                if (config.disableIdGeneration) return;
                const useNumberId = options.advanced?.database?.useNumberId;
                let generateId = options.advanced?.database?.generateId;
                if (options.advanced?.generateId !== undefined) {
                  logger.warn(
                    'Your Unblocked config includes advanced.generateId which is deprecated. Please use advanced.database.generateId instead. This will be removed in future releases.'
                  );
                  generateId = options.advanced?.generateId;
                }
                if (generateId === false || useNumberId) return;
                if (generateId) {
                  return generateId({
                    model,
                  });
